 ...
 */
public abstract class AbstractMongoSQLStructJdbcType implements StructJdbcType {
    ...
    protected AbstractMongoSQLStructJdbcType(
    ...
    public <X> ValueExtractor<X> getExtractor(JavaType<X> javaType) {
        return new BasicExtractor<>( javaType, this ) {
            ...
            private X getObject(Object object, WrapperOptions options) throws SQLException {
                ...
                return ( (AbstractMongoSQLStructJdbcType) getJdbcType() ).fromString(
                        object.toString(),
                        getJavaType(),
                        options
                );
                ...
                case '"':
                    if ( inQuote ) {
                        ...
                        if ( string.charAt( i + 1 ) == '(' ) {
                            ...
                            final JdbcMapping jdbcMapping = getJdbcValueSelectable( column ).getJdbcMapping();
                            if ( jdbcMapping.getJdbcType() instanceof AbstractMongoSQLStructJdbcType) {
                                final AbstractMongoSQLStructJdbcType structJdbcType;
                                structJdbcType = (AbstractMongoSQLStructJdbcType) jdbcMapping.getJdbcType();
            ...
            switch ( c ) {
                ...
                case '"':
                    if ( inQuote ) {
                        ...
                        if ( string.charAt( i + 1 ) == '(' ) {
                            // This could be a nested struct
                            if ( elementType.getJdbcType() instanceof AbstractMongoSQLStructJdbcType) {
                                final AbstractMongoSQLStructJdbcType structJdbcType;
                                structJdbcType = (AbstractMongoSQLStructJdbcType) elementType.getJdbcType();
                                final Object[] subValues = new Object[structJdbcType.embeddableMappingType.getJdbcValueCount()];
                                final int subEnd = structJdbcType.deserializeStruct(
                                        string,
                                        i + 1,
    ...
    protected <X> String toString(X value, JavaType<X> javaType, WrapperOptions options) throws SQLException {
        ...
        final StringBuilder sb = new StringBuilder();
        serializeStructTo( new MongoSQLAppender( sb ), value, options );
        return sb.toString();
    }

    private void serializeStructTo(MongoSQLAppender appender, Object value, WrapperOptions options) throws SQLException {
        serializeDomainValueTo( appender, options, value, '(' );
        appender.append( ')' );
    }

    private void serializeDomainValueTo(
            MongoSQLAppender appender,
    ...
    private void serializeJdbcValuesTo(
        ...
        for ( int i = 0; i < jdbcValues.length; i++ ) {
            ...
            if ( jdbcMapping.getJdbcType() instanceof AbstractMongoSQLStructJdbcType) {
                appender.quoteStart();
                ( (AbstractMongoSQLStructJdbcType) jdbcMapping.getJdbcType() ).serializeJdbcValuesTo(
                        appender,
                        options,
                        (Object[]) jdbcValue,
                        '('
    ...
    private void serializeConvertedBasicTo(
        ...
        switch ( jdbcMapping.getJdbcType().getDefaultSqlTypeCode() ) {
            ...
            case SqlTypes.STRUCT:
                if ( subValue != null ) {
                    final AbstractMongoSQLStructJdbcType structJdbcType = (AbstractMongoSQLStructJdbcType) jdbcMapping.getJdbcType();
                    appender.quoteStart();
    ...
    private static class MongoSQLAppender extends StringBuilderSqlAppender {

        private int quote = 1;

        public MongoSQLAppender(StringBuilder sb) {
            super( sb );
        }
        ...
        @Override
        public MongoSQLAppender append(char fragment) {
            if ( quote != 1 ) {
        ...
        @Override
        public MongoSQLAppender append(CharSequence csq) {
            return append( csq, 0, csq.length() );
        }

        @Override
        public MongoSQLAppender append(CharSequence csq, int start, int end) {
            if ( quote != 1 ) {
                int len = end - start;
