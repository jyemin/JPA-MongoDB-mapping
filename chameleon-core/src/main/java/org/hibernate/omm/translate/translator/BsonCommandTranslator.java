...
import org.hibernate.omm.translate.translator.mongoast.AstElement;
import org.hibernate.omm.translate.translator.mongoast.AstFieldUpdate;
import org.hibernate.omm.translate.translator.mongoast.AstInsertCommand;
import org.hibernate.omm.translate.translator.mongoast.AstValue;
import org.hibernate.omm.translate.translator.mongoast.DeleteCommand;
import org.hibernate.omm.translate.translator.mongoast.UpdateCommand;
import org.hibernate.omm.translate.translator.mongoast.filters.AstAndFilter;
...
import org.hibernate.omm.translate.translator.mongoast.filters.AstMatchesEverythingFilter;
...
public class BsonCommandTranslator<T extends JdbcOperation> extends AbstractBsonTranslator<T> {
  ...
  public BsonCommandTranslator(
      final SessionFactoryImplementor sessionFactory, final Statement statement) {
  ...
  private void renderInsertInto(final TableInsertStandard tableInsert) {
    ...
    String collectionName = mqlAstState.expect(
        AttachmentKeys.collectionName(), () -> renderIntoIntoAndTable(tableInsert));
    ...
    tableInsert.forEachValueBinding((columnPosition, columnValueBinding) -> {
      ...
      elements.add(
          new AstElement(columnValueBinding.getColumnReference().getColumnExpression(), value));
    });
  ...
  public void visitStandardTableDelete(final TableDeleteStandard tableDelete) {
    ...
    try {
      ...
      try {
        ...
        //                    // TODO: untested path
        //                }
        root = new DeleteCommand(
            tableDelete.getMutatingTable().getTableName(), new AstAndFilter(filters));
      } finally {
        getClauseStack().pop();
      }
    } finally {
  ...
  protected void visitDeleteStatementOnly(final DeleteStatement statement) {
    if (CollectionUtil.isNotEmpty(statement.getReturningColumns())) {
      throw new NotSupportedRuntimeException(
          "delete statement with returning columns not supported");
    }
    ...
    if (statement.getFromClause().getRoots().get(0).hasRealJoins()) {
      throw new NotSupportedRuntimeException(
          "delete statement with root having real joins not supported");
    }
    renderDeleteClause(statement);
    AstFilter filter = statement.getRestriction() != null
        ? mqlAstState.expect(
            AttachmentKeys.filter(), () -> visitWhereClause(statement.getRestriction()))
  ...
  private void visitTableUpdate(
      final RestrictedTableMutation<? extends MutationOperation> tableUpdate) {
    ...
    try {
      int predicates =
          tableUpdate.getNumberOfKeyBindings() + tableUpdate.getNumberOfOptimisticLockBindings();
    ...
    root = new UpdateCommand(
        tableUpdate.getMutatingTable().getTableName(), new AstAndFilter(filters), updates);
  }
  ...
  protected void visitUpdateStatementOnly(final UpdateStatement statement) {
    ...
    if (statement.getFromClause().getRoots().get(0).hasRealJoins()) {
      throw new NotSupportedRuntimeException(
          "update statement with root having real joins not supported");
    }
    AstFilter filter = mqlAstState.expect(
        AttachmentKeys.filter(), () -> visitWhereClause(statement.getRestriction()));
    List<AstFieldUpdate> updates = mqlAstState.expect(
        AttachmentKeys.fieldUpdates(), () -> renderSetClause(statement.getAssignments()));
  ...
  protected void renderSetClause(final List<Assignment> assignments) {
    try {
      ...
      for (Assignment assignment : assignments) {
        updates.add(
            mqlAstState.expect(AttachmentKeys.fieldUpdate(), () -> visitSetAssignment(assignment)));
      }
  ...
  protected void visitSetAssignment(final Assignment assignment) {
    ...
    if (columnReferences.size() == 1) {
      ...
      AstValue value =
          mqlAstState.expect(AttachmentKeys.fieldValue(), () -> assignedValue.accept(this));
      mqlAstState.attach(
          AttachmentKeys.fieldUpdate(),
          new AstFieldUpdate(columnReference.getColumnExpression(), value));
    } else {
      throw new NotSupportedRuntimeException();
    }
  }
