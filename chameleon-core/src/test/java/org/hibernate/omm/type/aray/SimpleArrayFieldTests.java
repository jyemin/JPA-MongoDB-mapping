...
class SimpleArrayFieldTests {
  ...
  @Test
  @DisplayName(
      "when collection field was persisted, it should be saved as embedded array in MongoDB")
  ...
  @Test
  @DisplayName(
      "when collection field was queried by some entry, it should return result as expected")
  void test_query_mongo_embedded_array_field() {
    ...
    sessionFactory.inTransaction(session -> {
      ...
      var classicRomanticMovies = session
          .createQuery(queryStr, Movie.class)
